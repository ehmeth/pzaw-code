...
app.post("/add_card/:cardset_slug", (req, res) => {
...
app.get("/new_cardset", (req, res) => {
  res.render("cardset_new", {
    title: "Nowy zestaw",
  });
});

app.post("/new_cardset", (req, res) => {
  ...
  if (errors.length == 0) {
    flashcards.addCardset(cardset_slug, cardset_name);
    res.redirect(`/view/${cardset_slug}`);
  } else {
    res.status(400);
    res.render("cardset_new", {
...
app.get("/edit/:cardset_slug", (req, res) => {
  const cardset_slug = req.params.cardset_slug;
  const errors = [];
...
app.post("/edit/:cardset_slug", (req, res) => {
  const cardset_slug = req.params.cardset_slug;
  if (flashcards.hasCardset(cardset_slug)) {
    const cardset_name = req.body.name;
...
app.post("/edit/:cardset_slug/:card_id", (req, res) => {
  const cardset_slug = req.params.cardset_slug;
...
app.post("/delete/:cardset_slug/:card_id", (req, res) => {
  const cardset_slug = req.params.cardset_slug;
  const card_id = req.params.card_id;
  if (!flashcards.hasCardset(cardset_slug) || !flashcards.hasCard(card_id)) {
    res.sendStatus(404);
