...
const db_ops = {
  create_session: db.prepare(
    `INSERT INTO fc_session (id, user_id, csrf_token, created_at)
     VALUES (?, ?, ?, ?) RETURNING id, user_id, created_at, csrf_token;`,
  ),
  get_session: db.prepare(
    "SELECT id, user_id, csrf_token, created_at from fc_session WHERE id = ?;",
  ),
