...
def api_auth_register(request):
    ...
    # TODO(kleindan) validate username and password!

    if (get_user_model().objects.filter(username=data.get("username")).exists()):
        return JsonResponse({"message": "User already exists"}, status=HTTPStatus.BAD_REQUEST)
    user = get_user_model().objects.create_user(username=data.get('username'), email=None, password=data.get("password"))
    if user is not None:
        login(request, user)
        return JsonResponse({"message": "authenticated"})
    else:
        return JsonResponse({"message": "There was a problem creating user account"}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
...
def api_polls_vote(request, poll_id):
    try:
        data = json.loads(request.body)
        choice = Choice.objects.get(question_id=poll_id, pk=data['choiceId'])
        choice.votes += 1
        choice.save()
        response = {
            "message": "ok",
        ...
        return JsonResponse({"message": f"No answer with id {data['choiceId']}"}, status=HTTPStatus.NOT_FOUND)
    except Poll.MultipleObjectsReturned:
        return JsonResponse({"message": f"Multiple choices found."}, status=HTTPStatus.INTERNAL_SERVER_ERROR)

def api_polls_details(request, poll_id):
    try:
        poll = Poll.objects.get(pk=poll_id)
        poll_obj = { "question": poll.question_text}
        poll_obj['choices'] = [{
            "text": choice.choice_text,
            "id": choice.pk,
            "votes": choice.votes,
            } for choice in Choice.objects.filter(question_id=poll_id)]
        return JsonResponse({"poll": poll_obj})
    except Poll.DoesNotExist:
        return JsonResponse({"message": f"No poll with id {poll_id}"}, status=HTTPStatus.NOT_FOUND)
    except Poll.MultipleObjectsReturned:
        return JsonResponse({"message": f"Multiple polls found."}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
