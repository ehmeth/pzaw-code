  ...
  if (!flashcards.hasCardset(cardset_slug)) {
    ...
    if (!flashcards.canEdit(cardset_slug, res.locals.user)) {
      ...
      if (req.body.csrf_token !== res.locals.session.csrf_token) {
        errors.push("Błąd walidacji CSRF");
      }
      if (errors.length == 0) {
...
app.post("/delete/:cardset_slug/:card_id", auth.login_required, (req, res) => {
  ...
  if (!flashcards.hasCardset(cardset_slug) || !flashcards.hasCard(card_id)) {
    ...
    if (!flashcards.canEdit(cardset_slug, res.locals.user)) {
      res.status(401);
      res.redirect("/");
    } else if (req.body.csrf_token !== res.locals.session.csrf_token) {
      res.status(401);
      res.redirect("/");
    } else {
...
app.get("/quiz", (req, res) => {
  res.render("quiz", {
    title: "Quiz",
    siteScripts: ["js/quiz.js"],
  });
});

app.get("/study/:cardset_slug", (req, res) => {

  res.render("study", {
    title: "Nauka",
    siteScripts: ["/js/study.js"],
  });
});

app.get("/api/cardsets", (req, res) => {
  res.send(flashcards.getCardsetSummaries());
});

app.get("/api/cardset/:cardset_slug", (req, res) => {
  const cardset = flashcards.getCardset(req.params.cardset_slug);
  if (cardset) {
    res.send(cardset);
  } else {
    res.status(404).send({"message": "Cardset not found"})
  }
});
